...
import numpy as np
from proposals import PGCNDataSet
from ragcn_model import PGCN
from opts import parser
from features.ops import CompletenessLoss, ClassWiseRegressionLoss
from features.utils import get_and_save_args, get_logger
...
def train(train_loader, model, act_criterion, comp_criterion, regression_criterion, optimizer, epoch):
    ...
    for i, (prop_fts, prop_type, prop_labels, prop_reg_targets) in enumerate(train_loader):
        # measure data_info loading time
        ...
        writer.add_scalar('data_info/loss', losses.val, epoch*len(train_loader)+i+1)
        writer.add_scalar('data_info/Reg_loss', reg_losses.val, epoch*len(train_loader)+i+1)
        writer.add_scalar('data_info/Act_loss', act_losses.val, epoch*len(train_loader)+i+1)
        writer.add_scalar('data_info/comp_loss', comp_losses.val, epoch*len(train_loader)+i+1)
