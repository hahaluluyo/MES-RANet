...
def _train(model, data_loader, optimizer, epoch, device, writer, opt,
    ...
    for batch_idx, (feature, micro_apex_score, macro_apex_score,
        ...
        loss_micro_action = _probability_loss(output_micro_action,
                                              micro_action_score,
                                              opt["tem_action_gamma"],
                                              opt["tem_action_alpha"],
                                              opt["tem_label_smooth"])
        ...
        with torch.no_grad():
            model._weight_args -= (
                opt["tem_training_lr"] * model._weight_args.grad
                + opt["tem_weight_decay"] * model._weight_args
            )
...
def tem_train_and_eval(opt):
    ...
    optimizer = configure_optimizers(model, opt["tem_training_lr"],
                                     opt["tem_weight_decay"])
    scheduler = torch.optim.lr_scheduler.ExponentialLR(
        optimizer, opt['tem_lr_scheduler'])

    def _result(type_idx):
        with torch.no_grad():
            _get_model_output_full(model, epoch, device, opt)
            tem_nms_once(opt, epoch)
            tem_iou_once(opt, epoch)

            anno_df = pd.read_csv(opt['anno_csv'])
            m = len(anno_df[anno_df['type_idx'] == type_idx])

            tem_final_result_csv = os.path.join(
                opt['project_root'], opt['output_dir_name'],
                'sub_tem_final_result', str(epoch).zfill(3) + '.csv')
            if os.path.exists(tem_final_result_csv) is not True:
                return 0, 0, 0, m
            ...
            df = pd.read_csv(tem_final_result_csv)
    ...
    for epoch in range(opt['epochs']):
        ...
        if opt['save_model'] is True:
            save_model(opt["model_save_root"], "tem_models",
...
def tem_train(opt, subject=None):
    device = opt['device'] if torch.cuda.is_available() else 'cpu'
    ...
    for epoch in range(opt['epochs']):
        ...
        if opt['verbose']:
            print("lr: ", scheduler.get_last_lr())
        if opt['save_model'] is True:
            save_model_per_subject(opt["model_save_root"], "tem_models",
                                   epoch, model, optimizer, subject)


def tem_train_group(opt, ca_subject):
    for subject in ca_subject:
        start_time = datetime.now()

        tem_train(opt, subject)
        ...
        if epoch >= epoch_begin:
            with torch.no_grad():
                ...
                weight_file = os.path.join(
                    opt["model_save_root"], subject, "tem_models",
...
def tem_output_group(opt, ca_subject):
    for subject in ca_subject:
        start_time = datetime.now()

        tem_output(opt, subject)
...
def tem_nms_once(opt, epoch, subject=None):
    if subject is not None:
        tem_nms_csv = os.path.join(
            opt['project_root'], opt['output_dir_name'],
            subject, 'tem_nms',
            str(epoch).zfill(3) + '.csv')
        tem_out_csv = os.path.join(
            opt['project_root'], opt['output_dir_name'],
            subject, "tem_out",
            str(epoch).zfill(3) + ".csv")
    else:
        tem_nms_csv = os.path.join(
            ...
            'tem_nms',
            str(epoch).zfill(3) + '.csv')
        dir_path, _ = os.path.split(tem_nms_csv)
    ...
    if os.path.exists(tem_nms_csv):
        os.remove(tem_nms_csv)

    if os.path.exists(tem_out_csv):
        df = pd.read_csv(tem_out_csv)
        # df["tp_fp"] = np.array(["FP"]*len(df))
        df = df.groupby(['video_name', "type_idx"]).apply(
            lambda x: nms(x, opt)).reset_index(drop=True)
        df.to_csv(tem_nms_csv, index=False)


def tem_nms(opt, subject):
    epoch_begin = opt['epoch_begin']

    for epoch in range(epoch_begin, opt['epochs']):
        tem_nms_once(opt, epoch, subject)

# 写入sub_tem_final_result
def tem_iou_once(opt, epoch, subject=None):
    if subject is not None:
        tem_final_result_csv = os.path.join(
        ...
        nms_csv = os.path.join(
            opt['project_root'], opt['output_dir_name'],
            subject, "tem_nms", str(epoch).zfill(3) + ".csv")
    else:
        tem_final_result_csv = os.path.join(
            opt['project_root'], opt['output_dir_name'],
            'sub_tem_final_result', str(epoch).zfill(3) + '.csv')
        dir_path, _ = os.path.split(tem_final_result_csv)
        if os.path.exists(dir_path) is not True:
            os.makedirs(dir_path)
        nms_csv = os.path.join(
            opt['project_root'], opt['output_dir_name'],
            "tem_nms", str(epoch).zfill(3) + ".csv")

    if os.path.exists(tem_final_result_csv):
        os.remove(tem_final_result_csv)
    ...
    if len(new_df) > 0:
        new_df.to_csv(tem_final_result_csv, index=False)
    return new_df


def tem_iou_process(opt, subject=None):
    ...
    for epoch in range(epoch_begin, opt['epochs']):
        ...
        new_df = tem_iou_once(opt, epoch, subject)
...
def tem_final_result_per_subject(opt, subject_list, type_idx=2):
    if type_idx == 0:
...
def tem_final_result_best(opt, subject_list, type_idx=2):
    anno_df = pd.read_csv(opt['anno_csv'])
